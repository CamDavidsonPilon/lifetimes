...
class TestGammaGammaFitter():
    ...
    def test_conditional_expected_average_profit(self):
        ...
        expected = np.array([24.65, 18.91, 35.17, 35.17, 35.17, 71.46, 18.91, 35.17, 27.28,
                             35.17])  # from Hardie spreadsheet http://brucehardie.com/notes/025/
    ...
    def test_customer_lifetime_value_with_bgf(self):
        ...
        bgf.fit(cdnow_customers_with_monetary_value['frequency'], cdnow_customers_with_monetary_value['recency'],
                cdnow_customers_with_monetary_value['T'], iterative_fitting=3)
...
class TestBetaGammaFitter():
    ...
    def test_scaling_inputs_gives_same_or_similar_results(self):
        ...
        bgf_with_large_inputs.fit(cdnow_customers['frequency'], scale * cdnow_customers['recency'],
                                  scale * cdnow_customers['T'])
        assert bgf_with_large_inputs._scale < 1.

        assert abs(bgf_with_large_inputs.conditional_probability_alive(1, scale * 1,
                                                                       scale * 2) - bgf.conditional_probability_alive(1,
                                                                                                                      1,
                                                                                                                      2)) < 10e-5
        assert abs(bgf_with_large_inputs.conditional_probability_alive(1, scale * 2,
                                                                       scale * 10) - bgf.conditional_probability_alive(
            1, 2, 10)) < 10e-5
...
class TestModifiedBetaGammaFitter():
    ...
    def test_scaling_inputs_gives_same_or_similar_results(self):
        ...
        assert abs(mbgf_with_large_inputs.conditional_probability_alive(1, scale * 1,
                                                                        scale * 2) - mbgf.conditional_probability_alive(
            1, 1, 2)) < 10e-2
        assert abs(mbgf_with_large_inputs.conditional_probability_alive(1, scale * 2,
                                                                        scale * 10) - mbgf.conditional_probability_alive(
            1, 2, 10)) < 10e-2
