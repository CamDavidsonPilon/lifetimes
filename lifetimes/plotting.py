...
import numpy as np
import pandas as pd
from lifetimes.utils import calculate_alive_path, expected_cumulative_transactions, expected_cumulative_clv
...
def plot_calibration_purchases_vs_holdout_spend(
    model_purchases, 
    model_spend, 
    ...
    **kwargs
    ...
    """
    Plot calibration purchases vs holdout spend.

    This currently relies too much on the lifetimes.util calibration_and_holdout_data function.

    Parameters
    ----------
    model_purchases: lifetimes model
        A fitted lifetimes model for purchasing.
    model_spend: lifetimes model
        A fitted lifetimes model for spend.
    calibration_holdout_matrix: pandas DataFrame
        DataFrame from calibration_and_holdout_data function.
    duration_clv: int, optional
        Duration to calculate CLV for model_spend.
    freq: str, optional
        freq used to create calibration_holdout_matrix.
    kind: str, optional
        x-axis :"frequency_cal". Purchases in calibration period,
                 "recency_cal". Age of customer at last purchase,
                 "T_cal". Age of customer at the end of calibration period,
    ...
    summary = calibration_holdout_matrix.copy()
    summary_spend = summary.loc[summary['frequency_cal']>0].copy()
    
    #Get holdout duration
    duration_holdout = summary.iloc[0]["duration_holdout"]
    
    #Get holdout clv
    summary['clv_holdout'] = summary['frequency_holdout'] * summary['monetary_value_holdout']
    
    #Get model predictions
    summary['model_predictions'] = model_spend.customer_lifetime_value(model_purchases, 
                                                            summary_spend['frequency_cal'], 
                                                            summary_spend['recency_cal'], 
                                                            summary_spend['T_cal'], 
                                                            summary_spend['monetary_value_cal'], 
                                                            time=duration_clv, 
                                                            freq=freq)
    ...
    plt.title("Actual ({}) Purchases in Holdout Period vs ({}) Predicted Purchases".format(agg.capitalize(),
                                                                                           agg.capitalize()))
    plt.axvline(x=int(summary['frequency_cal'].mean()), color="r", linestyle="--")
    plt.xlabel(x_labels[kind])
    plt.ylabel("Average of Purchases in Holdout Period")
    plt.legend()
...
def plot_cumulative_clv(transaction_prediction_model, 
                        ...
                        **kwargs):
    """
    Plot a figure of the predicted and actual cumulative clv of users.
