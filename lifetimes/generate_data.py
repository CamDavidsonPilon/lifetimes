...
def bgbg_model(T, alpha, beta, gamma, delta, size=1):
    ...
    # Generate hidden parameters fo all costumers
    ps = stats.beta.rvs(alpha, beta, size=size)  # probability of converting while alive
    thetas = stats.beta.rvs(gamma, delta, size=size)  # probability of dying at the beginning of a time bin

    columns = ['T', 'conversion_instant', 'customer_id']

    df = pd.DataFrame(np.zeros((size, len(columns))), columns=columns)

    for i in range(size):
        p = ps[i]  # probability of converting
        theta = thetas[i]  # probability of dying
        ...
        # first possibility of purchasing (the first day)
        converts = np.random.random() <= p
        if converts:
            tc = 0
            df.ix[i] = int(T[i]), int(tc), int(i)
            continue
